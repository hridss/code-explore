...
import (
	...
	"github.com/hridss/code-explore/app/work"
...
var (
	defaultConf = "conf/default.conf"

	confPath = flag.String("conf", "", "please set code-explore conf path")

	version = flag.Bool("version", false, "code-explore version")

	upgrade = flag.Bool("upgrade", false, "code-explore upgrade")
...
` +
		"Author: phachon\r\n" +
		"Version: " + Version + "\r\n" +
		"Link: https://github.com/phachon/code-explore"
	fg.Println(logo)
}
...
func initConfig() {
	...
	// init name
	beego.AppConfig.Set("sys.name", "code-explore")
...
func checkUpgrade() {
	if *upgrade == true {
		logs.Info("Start checking whether code-explore needs upgrading.")
		...
		logs.Info("code-explore Database version：" + versionDb)
		logs.Info("code-explore Now version: " + Version)

		if versionDb == Version {
			logs.Info("code-explore does not need updating.")
		} else {
			logs.Info("code-explore start upgrading.")
			err := models.UpgradeModel.Start(versionDb)
			if err != nil {
				logs.Error("code-explore upgrade failed.")
				os.Exit(1)
			}
			logs.Info("code-explore upgrade finish.")
		}
		os.Exit(0)
	}
}
